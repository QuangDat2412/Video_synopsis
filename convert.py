...
class Yolo4(object):
    ...
    def load_yolo(self):
        ...
        self.yolo4_model = yolo4_body(
            Input(shape=(416, 416, 3)), num_anchors//3, num_classes)
        ...
        for i in range(len(convs_sorted)):
            ...
            if i == 93 or i == 101 or i == 109:
                # no bn, with bias
                weights_shape = self.yolo4_model.layers[convs_sorted[i][1]].get_weights()[
                    0].shape
                bias_shape = self.yolo4_model.layers[convs_sorted[i][1]].get_weights()[
                    0].shape[3]
                filters = bias_shape
                ...
                weights_shape = self.yolo4_model.layers[convs_sorted[i][1]].get_weights()[
                    0].shape
                size = weights_shape[0]
                bn_shape = self.yolo4_model.layers[bns_sorted[bn_index][1]].get_weights()[
                    0].shape
                filters = bn_shape[0]
                darknet_w_shape = (filters, weights_shape[2], size, size)
                weights_size = np.product(weights_shape)
                ...
                conv_weights = np.ndarray(
                    ...
                    buffer=weights_file.read(weights_size * 4))
                conv_weights = np.transpose(conv_weights, [2, 3, 1, 0])
                self.yolo4_model.layers[convs_sorted[i]
                                        [1]].set_weights([conv_weights])
...
if __name__ == '__main__':
    ...
    yolo4_model = Yolo4(score, iou, anchors_path,
                        classes_path, model_path, weights_path)
