...
def poisson_integration(background_img, object_image, center, style=cv2.MIXED_CLONE):
    ...
    poisson_image = cv2.seamlessClone(
        object_image, background_img, mask, center, style)
...
def create_video():
    ...
    out = cv2.VideoWriter('./temp_file/last_result-mixed.avi',
                          fourcc, 30, (video_width, video_height))

    # 读入 new_index.json 文件
    index_json = open('./temp_file/new_index.json', 'r')
    video_index = json.load(index_json)

    background_img = cv2.imread('./temp_file/background.jpg')
    ...
    for frame in video_index:
        ...
        for loc_json in frame['locations']:
            ...
            frame_image = poisson_integration(frame_image, get_img_obj(
                location), location.get_bigger_centeroid())
            count += 1
