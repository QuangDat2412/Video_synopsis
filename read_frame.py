...
class ReadFrame:
    ...
    def get_targets(self):
        ...
        for target in self.targets:
            # 去掉空字符串和回车字符串
            li = [integer for integer in target.split(
            ...
            if id in target_set:
                target_list[id].add_frame(
                    frame_index=self.frame_index, position=position)
...
def get_targets_image(lines):
    ...
    while True:
        ...
        for loc_str in locations_str:
            quintuples = [integer for integer in loc_str.split(
                ' ') if integer and integer != '\n']
            ...
            dst = frame[bigger_tuple[1]:bigger_tuple[3],
                        bigger_tuple[0]:bigger_tuple[2]]

            folder_name = './temp_file/targets_image/target_' + str(target_id)
            img_name = folder_name + '/' + str(frame_index) + '-' + str(temp_position.top_left_x) + '-' + str(temp_position.top_left_y) \
                + '-' + str(temp_position.bottom_right_x) + '-' + \
                str(temp_position.bottom_right_y) + '.jpg'
...
def test():
    ...
    frame_txt = open('./temp_file/tracking.txt')
    frame_index = 1
    lines = []
    ...
    targets_json.write(json.dumps(
        target_list, default=lambda obj: obj.__dict__, sort_keys=True, indent=4))
